...
impl SettingsModal {
    ...
    pub fn cursor_left(&mut self) {
        if self.cursor_pos > 0 {
            self.cursor_pos -= 1;
            while self.cursor_pos > 0 && !self.edit_buffer.is_char_boundary(self.cursor_pos) {
                self.cursor_pos -= 1;
            }
        }
    }
    ...
    pub fn cursor_right(&mut self) {
        if self.cursor_pos < self.edit_buffer.len() {
            self.cursor_pos += 1;
            while self.cursor_pos < self.edit_buffer.len()
                && !self.edit_buffer.is_char_boundary(self.cursor_pos)
            {
                self.cursor_pos += 1;
            }
        }
        ...
        match row {
            ...
            | SettingsRow::OpenAiApiKey => {
                ...
                let line = Line::from(vec![
                    Span::styled(format!("{:>16}: ", row.label()), label_style),
                    Span::styled(
                        format!("[{:<24}]", display_value.chars().take(24).collect::<String>()),
                        value_style,
                    ),
                    status,
                ]);

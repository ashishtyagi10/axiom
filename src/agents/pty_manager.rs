...
use crate::agents::pty_agent::PtyAgent;
...
impl PtyAgentManager {
    ...
    /// Get the screen lines for a CLI agent
    pub fn get_screen_lines(&self, id: AgentId) -> Option<Vec<ratatui::text::Line<'static>>> {
        self.agents.get(&id).map(|agent| agent.get_screen_lines())
    }
    ...
    /// Get the currently focused CLI agent ID (if any)
    pub fn get_active(&self, id: AgentId) -> Option<&PtyAgent> {
        self.agents.get(&id)
    }

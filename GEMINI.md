...
*   **Panel System**: The UI is composed of panels implementing the `Panel` trait (`src/panels/mod.rs`).
    ...
    *   `ModelSelector`: Modal for switching LLM models.
...
4.  **Async Operations**: Long-running tasks (LLM requests) run in background threads and send results back via the `EventBus`.

### LLM Integration
*   **Provider**: `LlmProvider` trait (`src/llm/mod.rs`) defines the interface. Currently, `OllamaProvider` (`src/llm/ollama.rs`) is implemented.
